...
- INERTIA: sum of squared distance from each point to its cluster $\sum_{i=1}^{n} (x_i-C_k)^2$
...
- DISTORTION: average of squared distance from each point $x_i$ to its cluster $C_k$ $\frac{1}{n}\sum_{i=1}^{n} (x_i-c_k)^2$
